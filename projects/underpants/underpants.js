...
_.unique = function(array){
    ...
    if (_.indexOf(array, array[i]) === i){ // 
        unique.push(array[_.indexOf(array, array[i])]); //pushing values from array to unique (only exact copy of array is created)
...
_.filter = function(array, func){
    let newArray = [];
   _.each(array, function(ele, i, array){ // for(let i=0; i < array.length; i++){
        if(func(ele, i, array)){
        newArray.push(ele);
        }
    })
